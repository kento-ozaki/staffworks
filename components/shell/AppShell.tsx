...
function isAuthFreePath(pathname: string): boolean {
  // ログイン不要（＝シェルやヘッダーを出さない）ページ
  // ※ basePath は Next 側で除去された pathname が入る前提
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  // ログイン画面・パスワード変更画面・オープニング画面・利用規約は「ヘッダー含むシェル」を出さない
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F1F1F1" }}>
      ...
      >
        {/* Content Shell */}
        <div style={{ width: "100%", maxWidth: "47.9375rem", minWidth: "20rem", margin: "0 auto" }}>
          {children}
        </div>
      </main>
